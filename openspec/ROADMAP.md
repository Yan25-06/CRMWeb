...
| 10 | `add-admin-panel` | admin-panel | 2, 9 |

## Ghi chú quan trọng

**Thứ tự cố định:**
- #2 (auth) phải trước #3 (RLS) — cần session thật để test phân tách dữ liệu
- #4–#9 là service layer, chạy song song sau khi #3 xong
- #9 là change **cutover**: xóa `src/store/db.js` và logic localStorage; cần làm cuối cùng trong nhóm service layer
- #10 (Admin Panel) là change cuối, cần đủ #2 + #9

**Scope từng nhóm service layer (#4–#9):**
...
- #9: `mock_tests`, `mock_test_results`, `settings` → banner mất mạng + retry → **xóa `db.js`**
