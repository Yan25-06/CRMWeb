...
```typescript
{
  ...
  maxStudents?: number
  skillConfig: Array<{ name: string; order: number }>
                      // Cấu hình kỹ năng của lớp — CHỈ tên và thứ tự, KHÔNG có maxScore;
                      // fallback về IELTS 4 kỹ năng khi null/rỗng.
                      // Giá trị maxScore còn sót trong dữ liệu cũ bị bỏ qua ở application layer.
  createdAt: number
}
```
...
```typescript
{
  ...
  scoreMax: { [skillName: string]: number }
                      // Snapshot thang điểm tối đa từ mock test gần nhất tại thời điểm tạo
                      // (vd { "Listening": 9, "Reading": 9 }). Phiếu cũ mặc định {} → fallback 9.
  tags?: string[]
