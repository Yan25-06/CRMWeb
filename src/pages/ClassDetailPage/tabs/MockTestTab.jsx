...
import { getMockTestsByClass, getMockTestResultsByTest, deleteMockTest, getResultsByStudent } from '@/services/mockTestService'
  ...
  return (
    <div className="border border-navy-100 rounded-2xl overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3 bg-white hover:bg-navy-50/30 transition-colors">
        <button className="flex-1 flex items-center gap-3 text-left" onClick={() => setExpanded(o => !o)}>
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-navy-800">{mockTest.title}</p>
            <p className="text-xs text-navy-400 mt-0.5">
              {fmt(mockTest.date)} · {sections.length} phần thi
              {avgTotal !== null && (
                <span className={clsx('ml-2 font-semibold', avgPct >= 80 ? 'text-emerald-600' : avgPct >= 50 ? 'text-amber-600' : 'text-red-600')}>
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={() => exportMockTestExcel(mockTest, results, students, className)} className="p-1.5 text-navy-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Xuất Excel">
            <FileSpreadsheet size={15} />
          </button>
          <button onClick={() => onEdit(mockTest)} className="p-1.5 text-navy-400 hover:text-navy-700 hover:bg-navy-100 rounded-lg transition-colors" title="Chỉnh sửa">
            <Pencil size={15} />
          </button>
          <button onClick={() => onDelete(mockTest)} className="p-1.5 text-navy-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Xóa">
            <Trash2 size={15} />
          </button>
        </div>
      </div>
      {expanded && (
        <div className="border-t border-navy-50">
          <MockTestScoreTable mockTest={mockTest} results={results} students={students} onResultChange={onResultChange} />
        </div>
      )}
    </div>
  )
...
export const MockTestTab = ({ classId, className }) => {
  ...
  const [latestResultByStudent, setLatestResultByStudent] = useState({})
  ...
  const loadData = useCallback(async () => {
    const [tests, enrollments, allStudents, stgs] = await Promise.all([
      getMockTestsByClass(classId),
      getEnrollmentsByClass(classId),
      getStudents(),
      getSettings(),
    ])
    const activeStudents = allStudents.filter(s => enrollments.filter(e => e.status === 'active').some(e => e.studentId === s.id))
    setStudents(activeStudents)
    setMockTests(tests)
    setSettings(stgs)

    const byTest = {}
    await Promise.all(tests.map(async t => { byTest[t.id] = await getMockTestResultsByTest(t.id) }))
    setResultsByTest(byTest)

    const latestMap = {}
    await Promise.all(activeStudents.map(async stu => {
      const results = await getResultsByStudent(stu.id, classId)
      latestMap[stu.id] = results.find(r => r.totalScore > 0) ?? null
    }))
    setLatestResultByStudent(latestMap)
  }, [classId])
  ...
  useEffect(() => {
    ...
    getResultsByStudent(selectedStudentId, classId).then(setStudentResults)
  ...
  const handleModalSaved = async () => { setEditingTest(null); await loadData() }
  const handleResultChange = async () => { await loadData() }
    ...
    <div className="flex gap-4 h-full min-h-0">
      <div className="w-56 shrink-0 flex flex-col h-full bg-white border border-navy-100 rounded-2xl overflow-hidden shadow-navy-sm">
        ...
        <button
          ...
          className={clsx('w-full flex items-center gap-3 px-4 py-3 text-left transition-all border-b border-navy-50 border-l-2',
        ...
        {selectedStudentId === null ? (
          <>
            <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-navy-100 shadow-navy-sm">
              ...
              <Button variant="primary" size="sm" onClick={() => { setEditingTest(null); setModalOpen(true) }}>
                <Plus size={14} className="mr-1" /> Tạo Mock Test mới
              </Button>
            </div>
            {mockTests.length === 0 ? (
              <Card className="p-16 flex flex-col items-center justify-center text-center gap-3">
                ...
                <Button onClick={() => { setEditingTest(null); setModalOpen(true) }} className="mt-2">+ Tạo Mock Test mới</Button>
              </Card>
            ) : (
          ...
          <>
            <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-navy-100 shadow-navy-sm">
              <button onClick={() => setSelectedStudentId(null)} className="flex items-center gap-1.5 text-sm text-navy-500 hover:text-navy-800 transition-colors font-medium">
                ← Tổng quan lớp
              </button>
