...
import { Users, TrendingUp, Calendar, DollarSign, BookOpen, Clock } from 'lucide-react'
...
export const DashboardPage = ({ year, month, onNavigate }) => {
  const [stats, setStats] = useState({ totalStudents: 0, totalClasses: 0, presentToday: 0, monthlyRevenue: 0, yearlyRevenue: 0 })
  ...
  useEffect(() => {
    Promise.all([getStudents(), getClasses()]).then(([s, c]) => { setStudents(s); setClasses(c) })
  }, [])
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Học Sinh" value={stats.totalStudents} sub={`${stats.totalClasses} lớp`} icon={<Users size={16} />} accent="navy" />
        <StatCard label="Có Mặt Hôm Nay" value={stats.presentToday} sub={`trong ${stats.totalStudents} học sinh`} icon={<Calendar size={16} />} accent="success" />
        <StatCard label={`Doanh Thu ${monthName}`} value={fmt(stats.monthlyRevenue)} sub="tổng học phí tháng" icon={<DollarSign size={16} />} accent="warning" />
        <StatCard label={`Doanh Thu ${year}`} value={fmt(stats.yearlyRevenue)} sub="tổng cả năm" icon={<TrendingUp size={16} />} accent="navy" />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="overflow-hidden">
          <div className="px-5 py-4 border-b border-navy-50 flex items-center justify-between">
            ...
            <button onClick={() => onNavigate('students')} className="text-xs text-navy-500 hover:text-navy-800 font-medium transition-colors">Xem tất cả →</button>
          </div>
          {students.length === 0 ? (
            <div className="px-5 py-10 text-center text-navy-400 text-sm">Chưa có học sinh nào</div>
          ) : (
        ...
        <Card className="overflow-hidden">
          <div className="px-5 py-4 border-b border-navy-50 flex items-center justify-between">
            ...
            <button onClick={() => onNavigate('classes')} className="text-xs text-navy-500 hover:text-navy-800 font-medium transition-colors">Quản lý →</button>
          </div>
          {classes.length === 0 ? (
            <div className="px-5 py-10 text-center text-navy-400 text-sm">Chưa có lớp nào</div>
          ) : (
            <div className="divide-y divide-navy-50">
              {classes.map(cls => (
                <div key={cls.id} className="flex items-center gap-3 px-5 py-3 hover:bg-navy-50 transition-colors cursor-pointer" onClick={() => onNavigate('classes')}>
