...
export const StudentAttendancePanel = ({ student, classId, onClose }) => {
  ...
  const history = sessions.map(session => {
    ...
    return { session, present: att ? att.present : null, note: att ? att.note : '' }
  })
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 pointer-events-none">
        <div className={clsx("bg-white shadow-navy-2xl rounded-2xl flex flex-col w-full max-w-xl max-h-[90vh] pointer-events-auto", "animate-scale-in")}>
          ...
          <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
            ...
            <div>
              <h3 className="text-sm font-semibold text-navy-800 mb-3 uppercase tracking-wide">Lịch sử điểm danh</h3>
                        ...
                        <tr key={h.session.id} className="hover:bg-navy-50/50 transition-colors">
                          <td className="px-4 py-3">
                            <div className="flex flex-col gap-0.5">
                              ...
                              {h.session.topic && <span className="text-xs text-navy-500 line-clamp-1">{h.session.topic}</span>}
                              {h.note && <span className="text-xs text-amber-600 line-clamp-1 italic">Ghi chú: {h.note}</span>}
                            </div>
                          </td>
