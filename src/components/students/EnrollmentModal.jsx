...
import { getEnrollmentsByClass, upsertEnrollment } from '@/services/enrollmentService'
...
export const EnrollmentModal = ({
  ...
  useEffect(() => {
    if (!open) return
    if (mode === 'add') {
      Promise.all([getStudents(), getEnrollmentsByClass(classId)]).then(([allStudents, enrollments]) => {
    ...
    try {
      if (mode === 'add') {
        if (addSubMode === 'existing') {
          if (!selectedStudentId) { setLoading(false); return }
          ...
          if (!validateNew()) { setLoading(false); return }
        ...
        await upsertEnrollment(updated)
      }
      onSaved?.()
      onClose?.()
    } catch (err) {
  ...
  const submitDisabled =
    loading ||
    (mode === 'add' && (addSubMode === 'existing' ? !selectedStudentId : !newForm.name.trim()))
        ...
        <div className="px-6 py-5 flex flex-col gap-4 max-h-[72vh] overflow-y-auto">
            ...
            <div className="flex gap-1 p-1 bg-navy-50 rounded-xl">
              <button
                ...
                className={clsx('flex-1 py-1.5 text-xs font-medium rounded-lg transition-all',
                  addSubMode === 'existing' ? 'bg-white shadow-sm text-navy-800' : 'text-navy-500 hover:text-navy-700')}
              >
              ...
              <button
                ...
                className={clsx('flex-1 py-1.5 text-xs font-medium rounded-lg transition-all',
                  addSubMode === 'new' ? 'bg-white shadow-sm text-navy-800' : 'text-navy-500 hover:text-navy-700')}
              >
                Tạo học viên mới
              </button>
            </div>
          ...
          {mode === 'add' && addSubMode === 'existing' && (
            <div className="flex flex-col gap-2">
              <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Chọn học viên</label>
              {availableStudents.length === 0 ? (
                <p className="text-sm text-navy-400 text-center py-4">Tất cả học viên đã được thêm vào lớp</p>
              ) : (
                <>
                  <input
                    type="text"
                      ...
                      filteredStudents.map(s => (
                        <button
                          ...
                          className={clsx(
                            ...
                            selectedStudentId === s.id ? 'bg-navy-50 text-navy-800' : 'hover:bg-navy-50/50 text-navy-700'
                          )}
                        >
                  ...
                  {selectedStudentId && (
                    <div className="flex flex-col gap-1 pt-1">
                      <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Học phí / buổi (VNĐ)</label>
                      <input
                        type="number"
                        value={feePerSession}
          ...
          {mode === 'add' && addSubMode === 'new' && (
            <div className="flex flex-col gap-3">
              <p className="text-xs text-navy-400">Tạo học viên mới và tự động thêm vào lớp này</p>
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Họ và tên <span className="text-red-400 normal-case">*</span></label>
                <input
                  type="text" name="name" value={newForm.name} onChange={handleNewFormChange}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Số điện thoại</label>
                  <input type="text" name="phone" value={newForm.phone} onChange={handleNewFormChange} placeholder="SĐT phụ huynh" className="input text-sm" />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Khối</label>
                  <input type="text" name="grade" value={newForm.grade} onChange={handleNewFormChange} placeholder="VD: Lớp 5" className="input text-sm" />
                </div>
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Học phí / buổi (VNĐ)</label>
                <input type="number" name="feePerSession" value={newForm.feePerSession} onChange={handleNewFormChange} placeholder="VD: 150000" min="0" step="1000" className="input text-sm" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Ghi chú</label>
                <input type="text" name="note" value={newForm.note} onChange={handleNewFormChange} placeholder="Ghi chú thêm..." className="input text-sm" />
              </div>
          ...
          {mode === 'edit' && (
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Trạng thái</label>
              <select value={status} onChange={e => handleStatusChange(e.target.value)} className="select text-sm">
          ...
          {mode === 'edit' && (
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Học phí / buổi (VNĐ)</label>
              <input type="number" value={feePerSession} onChange={e => setFeePerSession(e.target.value)} placeholder="VD: 150000" min="0" step="1000" className="input text-sm" />
              <p className="text-xs text-navy-400">Học phí riêng của học viên này trong lớp này</p>
            </div>
          )}

          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Mục tiêu</label>
            <textarea value={goal} onChange={e => setGoal(e.target.value)} placeholder="VD: Đạt 7.0 IELTS để du học..." rows={2} className="input resize-none text-sm" />
          </div>

          {(mode === 'edit' || addSubMode === 'existing') && (
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-navy-600 uppercase tracking-wide">Ghi chú nội bộ</label>
              <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Ghi chú dành riêng cho GV..." rows={2} className="input resize-none text-sm" />
            </div>
          )}

          {confirmDrop && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex flex-col gap-3">
              <p className="text-sm text-amber-800 font-medium">Học viên này sẽ không nhận bài tập mới. Tiếp tục?</p>
              <div className="flex gap-2">
                <Button size="sm" variant="danger" onClick={confirmDropHandler} className="flex-1">Xác nhận cho nghỉ</Button>
                <Button size="sm" variant="secondary" onClick={() => { setConfirmDrop(false); setPendingStatus(null) }} className="flex-1">Hủy</Button>
              </div>
            </div>
          )}
        </div>
