...
import { getHomeworkByStudent } from '@/services/homeworkService'
...
export const StudentHomeworkPanel = ({ student, classId, onClose }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getEnrollment(student.id, classId),
    ]).then(([sess, hws, enroll]) => {
      setSessions(sess)
      setHomeworks(hws)
      setEnrolledDate(enroll?.enrolledAt ? enroll.enrolledAt.split('T')[0] : null)
    })
  }, [student?.id, classId])
  ...
  let doneCount = 0, inProgressCount = 0, notDoneCount = 0
  ...
  return (
    <>
      <div className="fixed inset-0 bg-navy-950/40 backdrop-blur-sm z-40 animate-fade-in" onClick={onClose} />
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 pointer-events-none">
        <div className={clsx("bg-white shadow-navy-2xl rounded-2xl flex flex-col w-full max-w-xl max-h-[90vh] pointer-events-auto", "animate-scale-in")}>
          ...
          <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
            ...
            <div>
              <h3 className="text-sm font-semibold text-navy-800 mb-3 uppercase tracking-wide">Lịch sử bài tập</h3>
              <div className="border border-navy-100 rounded-xl overflow-hidden">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-navy-50">
                    {history.length === 0 ? (
                      <tr><td colSpan={2} className="px-4 py-6 text-center text-navy-400">Chưa có dữ liệu</td></tr>
                    ) : (
                      history.map((h, i) => (
                        <tr key={h.session.id} className="hover:bg-navy-50/50 transition-colors">
                          <td className="px-4 py-3">
                            <div className="flex flex-col gap-0.5">
                              <span className="font-medium text-navy-800">Buổi {history.length - i} · {formatDate(h.session.date)}</span>
                              {(h.session.topic || h.hw?.title) && (
                                <span className="text-xs text-navy-500 line-clamp-1">{h.session.topic || h.hw?.title}</span>
                              )}
                              {h.hw?.note && <span className="text-xs text-navy-400 line-clamp-1 italic mt-1">Ghi chú: {h.hw.note}</span>}
                            </div>
                          </td>
