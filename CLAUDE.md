...
- **ScheduleCard (chấm công):** chip ở góc phải = trạng thái (mặc định "Đã dạy" xanh; bấm → "Vắng" đỏ + viền trái đỏ qua `att.bar`). Khi "Vắng" hiện **ô ghi chú inline** trên thẻ (debounce → `handleSetAttendanceNote`). Props `onToggleAttendance(item)` + `onAttendanceNote(item, note)` (WeeklyGrid kèm `date`). Bấm thân thẻ = sửa lịch. Chỉ hiện khi `canCheckTeacherAttendance`. Giờ hiển thị qua `fmtTime` (cắt còn `HH:MM`).
