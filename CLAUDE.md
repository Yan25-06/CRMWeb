...
- **Khớp mô hình học phí + đa GV (2026-09-18):** mỗi lớp ở BƯỚC 4 (`classes`) có `monthly_fee` thật (2500000/3000000/1800000/1500000); BƯỚC 5 (`enrollments`) không còn `fee_type/monthly_fee/course_fee`; BƯỚC 12 (`fees`) không còn `surcharge`, chỉ còn cờ `paid` mix true/false; bảng `payments` không còn được seed (INSERT đã xóa, cascade cleanup vẫn giữ). Ca T5 của lớp "IELTS Cơ Bản A1" (`c01`, phụ trách bởi `t1`) gán `schedule.teacher_id = t2` để minh hoạ tính năng chia buổi cho nhiều GV.
...
Services đã có: `studentService`, `classService` (+`teacherService`), `enrollmentService`, `sessionService`, `attendanceService`, `teacherAttendanceService`, `homeworkService`, `hwAssignmentService`, `submissionService`, `scheduleService`, `feeService`, `reviewService`, `sessionReviewService`, `generalCommentService`, `mockTestService`, `mockTestResultService`, `settingsService`, `curriculumService`. (`paymentService` đã **xóa** — mô hình học phí nay là tick nhị phân `fees.paid`, không còn ghi từng khoản thanh toán; xem "Model học phí (đã chốt)".) (`classMaterialService` vẫn còn file nhưng **orphan** — không còn UI import, xem "Model tài liệu giảng dạy CŨ" bên dưới.)
...
- `ClassDetailPage` có các tab: Students, Attendance, Homework, MockTest. Tab Students: nút "＋ Thêm" mở `BulkEnrollPickerModal` (chọn nhiều HS chưa có trong lớp → ghi danh hàng loạt qua `enrollMany`; không còn bước đặt học phí — học phí nay cố định ở `classes.monthly_fee`, sửa qua `ClassModal`); admin có thêm nút "Tạo mới" mở `EnrollmentModal` để tạo học sinh mới. Prop `initialTab` (default `'students'`) cho phép mở thẳng tab Attendance từ Dashboard. Truyền `scheduleTime={currentClass?.scheduleTime}` vào `AttendanceTab` và `HomeworkTab`; các tab tiếp tục truyền prop này vào `SessionModal` để tự điền giờ khi tạo buổi mới.
...
- **Admin Panel** (`AdminPanelPage`): route `/admin` (page `'admin'`), dải 4 `StatCard` tổng quan (Tổng học viên, Lớp đang hoạt động, Số giáo viên, HS chưa đóng phí tháng hiện tại — số chưa đóng dùng `feeService.buildFeesRows` rồi đếm số **học sinh duy nhất** có ít nhất 1 dòng `!r.paid`) + danh sách giáo viên + form tạo/giao/đổi lớp. Guard với `teacher.is_admin` (redirect về dashboard nếu không phải admin). Link "Admin" chỉ hiện trong Navbar khi `is_admin = true`. Card GV có nút "Xem lịch dạy" → expand danh sách lớp với `fmtDayList` + giờ + phòng.
- **FeesPage**: route `fees` (chỉ admin), 3 tab lọc theo trạng thái tick — "Tất cả"/"Chưa đóng"/"Đã đóng" (`PAYMENT_TABS`, không còn "Đóng một phần") + bộ chọn lớp (`classFilter`, mặc định `'all'`) lọc trước khi đếm tab. 2 `StatCard` đếm **người** — "Đã đóng đủ" (`paidCount/total` học viên) và "Chưa đóng" (`debtCount` học viên) qua `countFeeStudents` (không còn thẻ tổng tiền Tổng thu/Kỳ vọng/Còn nợ). `FeesTable` mỗi dòng có checkbox tick `paid` trực tiếp (`feeService.setPaid`, optimistic + revert khi lỗi), cột "Học phí" đọc từ `row.monthlyFee` (= `classes.monthly_fee`, cố định theo lớp, không sửa được ở đây).
...
- **Mô hình lương giáo viên** (migration `20260627000001_add_teacher_session_rate.sql`):
  ...
  - **`scheduled` tính theo GV của từng CA, không theo GV phụ trách lớp** (từ migration `20260918000002_multi_teacher_per_class.sql`): `buildPayrollRows` map mỗi `schedule` row về `scheduleTeacher = schedule.teacherId ?? classes.teacherId` (rỗng = GV phụ trách lớp dạy ca đó), rồi cộng `countWeekdayOccurrences(year, month, dayOfWeek)` vào đúng GV đó. Một lớp chia ca cho 2 GV (VD T2 cô A, T4 cô B) → lương/`scheduled` tách đúng theo người dạy từng ca, không dồn hết về GV phụ trách lớp.
  - Logic thuần (không side-effect) tập trung tại `src/utils/payroll.js` (`buildPayrollRows`, `countWeekdayOccurrences`); UI tại `src/components/schedule/PayrollTab.jsx`. Test: `node scripts/test-payroll.mjs`.
...
- **Teacher read-only `students` + `mock_tests`** (migration `20260605000001_restrict_teacher_students_mocktests.sql`): giáo viên thường **mất** INSERT/UPDATE/DELETE trên bảng `students` và `mock_tests` (đề Mock Test), chỉ còn SELECT. Drop 3 policy `"students: teacher insert/update/delete"` + 3 policy `"mock_tests: teacher insert/update/delete"`; giữ policy SELECT của teacher và toàn bộ policy admin. `mock_test_results` (nhập điểm) và mọi bảng nghiệp vụ khác (điểm danh, bài tập, nhận xét, `enrollments`...) **không đổi** — teacher vẫn ghi được. Rollback = re-create 6 policy đã drop (nội dung gốc trong `20260602000001`).
  ...
  - **Ghi danh (`EnrollmentModal`)**: khi `!isAdmin` ẩn toggle "Tạo học viên mới" → giáo viên chỉ gắn học sinh **đã tồn tại** vào lớp (admin phải tạo học sinh trước). Chế độ `edit` chỉ sửa field enrollment (status, mục tiêu, ghi chú), không đụng bảng `students`.
...
- **Quyền truy cập lớp khóa qua `can_access_class(cid)`, không còn `classes.teacher_id = auth.uid()` trực tiếp** (migration `20260918000002_multi_teacher_per_class.sql`): một lớp có thể chia buổi cho nhiều giáo viên (`schedule.teacher_id`) nên "truy cập được lớp" = GV phụ trách lớp HOẶC có ít nhất một ca (`schedule.teacher_id = auth.uid()`) trong lớp đó. Hàm SQL `security definer`, `grant execute` cho `authenticated`. Toàn bộ policy SELECT của `classes` và mọi bảng khóa theo `class_id` (`enrollments`, `sessions`, `hw_assignments`, `reviews`, `session_reviews`, `general_comments`, và gián tiếp `attendance`/`homeworks`/`submissions`/`mock_test_results` qua join) đã đổi từ điều kiện `teacher_id = auth.uid()` sang gọi hàm này. Xem mục "Model nhiều giáo viên / lớp" bên dưới.
...
- **Ghi danh hàng loạt:** `src/utils/enrollMany.js` (helper Promise.allSettled gọi `enrollmentService.upsert`), `BulkEnrollModal.jsx` (StudentsDirectoryPage), `BulkEnrollPickerModal.jsx` (ClassDetailPage tab Students). Không còn bước đặt học phí trong luồng ghi danh (`BulkFeeFields.jsx`/`BulkFeeModal.jsx` đã bị xóa ở Task 6 khi học phí chuyển sang cố định theo `classes.monthly_fee`).
...
- **Học phí là một Ô TICK theo tháng, không còn số tiền/lịch sử thanh toán ở tầng nhập liệu** (migration `20260918000001_simplify_fees_to_paid_flag.sql`, thay thế toàn bộ model "theo tháng/theo khóa + surcharge + payments" cũ).
- **Mức phí cố định ở CẤP LỚP — `classes.monthly_fee`** (integer, nullable): mọi học sinh trong cùng lớp đóng như nhau, không đặt riêng theo enrollment nữa. Đổi học phí của lớp = sửa 1 chỗ, áp dụng ngay cho toàn lớp.
- **`fees.paid`** (boolean) là trạng thái nhị phân DUY NHẤT — không còn "đóng một phần", không còn số tiền/ngày đóng/phương thức. Khóa theo `(student_id, class_id, year, month)` — vẫn tách riêng theo từng lớp như trước (kế thừa từ `20260714000001_scope_fees_payments_per_class.sql`).
- **Đã orphan/xóa theo migration này** (giữ nguyên tiền lệ không-drop-cột của repo):
  - `enrollments.fee_type` / `enrollments.monthly_fee` / `enrollments.course_fee` — cột còn trong DB nhưng app không đọc/ghi nữa.
  - `fees.surcharge` — cột còn trong DB nhưng orphan.
  - Bảng `payments` và **toàn bộ `paymentService`** — service đã **xóa file**, không còn UI nào ghi lịch sử thanh toán; bảng `payments` vẫn còn trong DB để tra cứu SQL lịch sử cũ nhưng không migration drop.
- **`feeService.js` chỉ còn 2 method:**
  - `buildFeesRows(year, month)`: JOIN `enrollments` (chưa `dropped`) với `classes` để lấy `monthlyFee`, JOIN `fees` để lấy cờ `paid` theo `(student_id, class_id)` → trả **1 dòng cho mỗi enrollment active** `{ studentId, classId, name, className, monthlyFee, paid }`.
  - `setPaid(studentId, classId, year, month, paid)`: `upsert` vào `fees` theo `onConflict: 'student_id,class_id,year,month'`.
- **Logic thuần (không side-effect) ở `src/utils/fees.js`** (test bằng `node scripts/test-fees.mjs`, không import React/supabase):
  - `filterFeeRows(rows, { className, status })`: lọc theo lớp + trạng thái (`all`/`debt`/`paid` — không còn `partial`).
  - `countFeeStudents(rows)`: đếm theo **học sinh duy nhất** — một học sinh chỉ tính "đã đóng đủ" khi MỌI lớp học đều đã tick `paid`; còn nợ ở bất kỳ lớp nào → tính "chưa đóng đủ". Dùng cho stat card Dashboard "Chưa đóng phí", AdminPanel "HS chưa đóng phí" (đếm `!r.paid` theo student, không còn so `paid < expected`), FeesPage "Đã đóng đủ"/"Chưa đóng".
- UI: `FeesPage` không còn ô nhập số tiền hay `PaymentModal` — mỗi dòng trong `FeesTable` là 1 checkbox tick `paid` trực tiếp (optimistic UI, revert khi lỗi). Đặt mức phí của lớp làm ở `ClassModal` (`classes.monthly_fee`), không làm ở `EnrollmentModal`.
...
## Model nhiều giáo viên / lớp (migration `20260918000002_multi_teacher_per_class.sql`)
- Một lớp có thể **chia buổi cho nhiều giáo viên** (VD lớp T2-T5: T2 cô A dạy, T5 cô B dạy) thay vì luôn do đúng 1 GV phụ trách lớp (`classes.teacher_id`) dạy mọi buổi.
- **`schedule.teacher_id`** (uuid, nullable, FK `teachers`, `on delete set null`): GV dạy CA đó.
  - `NULL` = GV phụ trách lớp (`classes.teacher_id`) dạy ca đó (mặc định, tương thích ngược với lớp không chia ca).
  - `<uid>` = giáo viên đó dạy ca đó, thay cho GV phụ trách lớp.
  - Đọc hiệu lực ở đâu cũng theo pattern `s.teacherId ?? classTeacherId` (fallback về GV phụ trách lớp) — xem `ClassDetailPage.classTeacherLines`, `SchedulePage` (nhiều chỗ), `payroll.js`.
- **`can_access_class(cid)`** — hàm SQL `security definer`, `stable`, thay cho điều kiện `classes.teacher_id = auth.uid()` cũ: trả `true` nếu `is_admin()` HOẶC GV phụ trách lớp HOẶC GV có ít nhất 1 ca (`schedule.teacher_id = auth.uid()`) trong lớp. Dùng làm điều kiện policy SELECT của `classes` và mọi bảng khóa theo `class_id` (`enrollments`, `sessions`, `hw_assignments`, `reviews`, `session_reviews`, `general_comments`) + gián tiếp qua join cho `attendance`/`homeworks` (qua `sessions.class_id`), `submissions` (qua `hw_assignments.class_id`), `mock_test_results` (qua `mock_tests.class_id`). Write của `classes`/`schedule` vẫn chỉ admin (lịch sinh tự động từ `ClassModal`, màn hình admin).
- **`students`**: policy SELECT mở rộng — GV đọc được học sinh nếu là chủ (`teacher_id = auth.uid()`) HOẶC có enrollment trong một lớp mà `can_access_class` trả `true` (kể cả lớp chỉ dạy 1-2 ca do chia sẻ).
- **UI gán GV theo ca**: `ClassModal` — khi `isAdmin`, mục "Giáo viên theo buổi" hiện 1 dropdown GV cho mỗi thứ đã chọn trong `scheduleDayList` (để trống = "Giáo viên phụ trách" = GV phụ trách lớp dạy). State `teacherByDay` gửi qua `classService.create/update` → `scheduleService.syncForClass(classId, { dayList, startTime, endTime, room, teacherByDay })`, ghi `schedule.teacher_id` theo từng thứ; `teacherByDay` **undefined** (không truyền) = giữ nguyên `teacher_id` cũ (không xoá khi update thứ khác không đụng đến GV).
- **`ClassDetailPage`**: header hiện `classTeacherLines` (gộp theo `teacherId`, liệt kê các thứ mỗi GV dạy) khi lớp có **>1 GV** — chỉ admin load được `teacherService.getAll()` để resolve tên đầy đủ, GV thường fallback `'Giáo viên'`.
- **Chấm công (`teacher_attendance`) ghi theo GV CỦA CA, không phải GV phụ trách lớp**: `SchedulePage` tính `slotTeacherId = item.teacherId ?? cls?.teacherId` trước mọi lần `teacherAttendanceService.upsert` — áp dụng cho cả 3 luồng: `handleToggleAttendance` (đổi trạng thái Đã dạy/Vắng), `handleSetAttendanceNote` (ghi chú), `handleSetSubstitute` (chọn GV dạy thay). Do `upsert` ghi full-row keyed theo `(schedule_id, date)`, 3 luồng đều PHẢI tự tính lại `slotTeacherId` — quên tính lại (gán cứng `cls.teacherId`) sẽ âm thầm ghi đè đúng GV của ca thành GV phụ trách lớp. RLS insert `teacher_attendance: teacher self insert` cũng đối chiếu `coalesce(s.teacher_id, c.teacher_id) = auth.uid()` — GV chỉ tự chấm công được ca của chính mình (theo `schedule.teacher_id`, fallback `classes.teacher_id`).
- **Lương** (`payroll.js`) tính `scheduled` theo GV của từng ca (không dồn về GV phụ trách lớp) — xem chi tiết ở mục "Mô hình lương giáo viên".
- **Rollback**: drop toàn bộ policy migration này tạo, re-create bản cũ (nội dung gốc ở `20260602000001`, `20260604000001`, `20260625000001`, `20260625000002`, `20260710000001`), rồi `drop function if exists public.can_access_class(uuid);` và `alter table public.schedule drop column if exists teacher_id;`.
