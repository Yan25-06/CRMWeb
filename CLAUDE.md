...
- **Chấm công giáo viên — opt-in 3 trạng thái**: `SchedulePage` load `teacherAttendanceService.getByWeek` cho tuần đang xem (khi `canCheckOwnAttendance` — mọi GV). 3 trạng thái: **Chưa xác nhận** (không có record) → **Đã dạy** (`status='present'`) → **Vắng** (`status='absent'`) → bấm tiếp xóa record (về Chưa xác nhận). Bảng `teacher_attendance` (migration `20260620000001`), unique `(schedule_id, date)`, RLS: admin full write + **GV tự chấm công lớp mình** (policy `teacher self insert/update/delete` từ migration `20260625000001`). Hằng số 3 trạng thái ở `src/components/schedule/attendanceStatus.js` (pending/present/absent). `TeacherAttendanceModal` đã xóa.
